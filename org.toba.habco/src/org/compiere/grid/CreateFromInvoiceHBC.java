 ...
 */
public abstract class CreateFromInvoiceHBC extends CreateFrom
{
	/**
	 *  Protected Constructor
	 *  @param mTab MTab
	 */
	public CreateFromInvoiceHBC(GridTab mTab)
	{
		super(mTab);
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue())
			{
				...
				if(M_Product_ID!=0)
				{
					product = MProduct.get(Env.getCtx(), M_Product_ID);
					//HBC 2590 @phie
					invoiceLine.set_ValueOfColumn("hbc_percentage_split_barge", product.get_Value("hbc_percentage_split_barge"));
					invoiceLine.set_ValueOfColumn("hbc_percentage_split_tugboat", product.get_Value("hbc_percentage_split_tugboat"));
					//end phie
				}
				...
				if (M_InOutLine_ID != 0)
				{
					...
					if (orderLine == null && inoutLine.getC_OrderLine_ID() != 0)
					{
						...
						MMatchPO matchPO = new Query(Env.getCtx(),MMatchPO.Table_Name, "M_InOutLine_ID=? AND C_OrderLine_ID!=0", trxName)
										.setParameters(M_InOutLine_ID)
										.first();
						
						if (matchPO!=null)
							orderLine = new MOrderLine (Env.getCtx(), matchPO.getC_OrderLine_ID(), trxName);
					}
...
//						}
				...
				M_PriceList_Version_ID=DB.getSQLValueEx(null, sqlString,orderLine.getC_Order().getM_PriceList_ID(), orderLine.getC_Order().getDateOrdered());
		 		
				MInOut inout = null;
				
				if(M_InOutLine_ID!=0)
				{
					inout = new MInOut(Env.getCtx(), inoutLine.getM_InOut_ID(), trxName);
					if(!p_order.get_ValueAsBoolean("IsFuelTrx"))
			 			M_PriceList_Version_ID=DB.getSQLValueEx(null, sqlString,orderLine.getC_Order().getM_PriceList_ID(), orderLine.getC_Order().getDateOrdered());
			 		else
			 			M_PriceList_Version_ID=DB.getSQLValueEx(null, sqlString,orderLine.getC_Order().getM_PriceList_ID(), inout.getDateReceived());
					
				}
				MProductPricing pricing = new MProductPricing (orderLine.getM_Product_ID(), orderLine.getC_Order().getC_BPartner_ID(), QtyEntered, orderLine.getC_Order().isSOTrx());
		 		int M_PriceList_ID = orderLine.getC_Order().getM_PriceList_ID();
		 		pricing.setM_PriceList_ID(M_PriceList_ID);
		 		
				pricing.setM_PriceList_Version_ID(M_PriceList_Version_ID);
		 		pricing.setPriceDate(orderLine.getC_Order().getDateOrdered());
		 		
		 		/* commented by TommyAng
				invoiceLine.setPrice(pricing.getPriceStd());
				invoiceLine.setPriceEntered(pricing.getPriceStd().subtract(pricing.getPriceStd().multiply(orderLine.getDiscount())));
				invoiceLine.setPriceActual(pricing.getPriceStd());
				invoiceLine.setPriceList(pricing.getPriceList());
				invoiceLine.setPriceLimit(pricing.getPriceLimit());
		 		*/
				...
				invoiceLine.setPriceActual(orderLine.getPriceEntered());
				invoiceLine.setPriceList(orderLine.getPriceList());
				invoiceLine.setPriceLimit(orderLine.getPriceLimit());
				invoiceLine.set_ValueOfColumn("HBC_Tugboat_ID", order.get_Value("HBC_Tugboat_ID"));
				invoiceLine.set_ValueOfColumn("HBC_Barge_ID", order.get_Value("HBC_Barge_ID"));
				//@phie
				invoiceLine.set_ValueOfColumn("C_Campaign_ID", orderLine.get_Value("C_Campaign_ID"));
				invoiceLine.setIsTrackAsAsset(orderLine.get_ValueAsBoolean("isTrackAsAsset"));
				
				
				if(M_InOutLine_ID!=0)
				{
					invoiceLine.set_ValueOfColumn("HBC_Tugboat_ID", inout.get_Value("HBC_Tugboat_ID"));
					invoiceLine.set_ValueOfColumn("HBC_Barge_ID", inout.get_Value("HBC_Barge_ID"));
					
					inoutLine = new MInOutLine (Env.getCtx(), M_InOutLine_ID, trxName);
					invoiceLine.setIsTrackAsAsset(inoutLine.get_ValueAsBoolean("isTrackAsAsset"));
					invoiceLine.set_ValueOfColumn("C_Campaign_ID", inoutLine.get_Value("C_Campaign_ID"));
				}//end phie
				...
				if(invoiceLine.get_ValueAsBoolean("isTrackAsAsset"))
				{
					...
					invoiceLine.setA_Asset_Group_ID(product.getM_Product_Category().getA_Asset_Group_ID());
				}//end phie 2730
				
				//@TommyAng
				//int M_PriceList_ID = Env.getContextAsInt(ctx, windowNo, "M_PriceList_ID");
				int StdPrecision = MPriceList.getStandardPrecision(invoiceLine.getCtx(), M_PriceList_ID);
				BigDecimal TaxAmt = Env.ZERO;
				//Integer taxID = (Integer)mTab.getValue("C_Tax_ID");
				Integer taxID = invoiceLine.getC_Tax_ID();
				if (taxID != null)
				{
					int C_Tax_ID = taxID.intValue();
					MTax tax = new MTax (invoiceLine.getCtx(), C_Tax_ID, null);
					TaxAmt = tax.calculateTax(invoiceLine.getLineNetAmt(), invoiceLine.isTaxIncluded(), StdPrecision);
					invoiceLine.setTaxAmt(TaxAmt);
				}
				invoiceLine.setLineTotalAmt(invoiceLine.getLineNetAmt().add(TaxAmt));
				//end @TommyAng
