 ...
 */
public abstract class CreateFromShipmentHBC extends CreateFrom 
{
	/**  Loaded Invoice             */
	 ...
	 *  @param mTab MTab
	 */
	public CreateFromShipmentHBC(GridTab mTab)
	{
		super(mTab);
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		int p_InOut_ID = Env.getContextAsInt(Env.getCtx(),getGridTab().getWindowNo(), "M_InOut_ID");
		
		Vector<Vector<Object>> data = new Vector<Vector<Object>>();
		StringBuilder sql = new StringBuilder("SELECT "
				+ "l.QtyOrdered-SUM(COALESCE(m.Qty,0))"
				// subtract drafted lines from this or other orders IDEMPIERE-2889
				+ "-COALESCE((SELECT SUM(MovementQty) FROM M_InOutLine iol WHERE iol.M_InOut_ID="+p_InOut_ID
				+ " AND l.C_OrderLine_ID=iol.C_OrderLine_ID ),0),"	//	1
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue()) {
				...
				if (C_OrderLine_ID != 0)
				{
					...
					//@phie HBC 2640
					iol.setIsTrackAsAsset(ol.get_ValueAsBoolean("isTrackAsAsset"));
					//end phie
					MOrder order = (MOrder) ol.getC_Order();
					iol.set_ValueOfColumn("HBC_Tugboat_ID", order.get_Value("HBC_Tugboat_ID"));
					iol.set_ValueOfColumn("HBC_Barge_ID", order.get_Value("HBC_Barge_ID"));
				}
				else if (il != null)
				{
