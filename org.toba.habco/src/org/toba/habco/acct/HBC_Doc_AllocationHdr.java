 ...
 *
 */
public class HBC_Doc_AllocationHdr extends Doc_AllocationHdr
{
	/**
	 *  Constructor
	 ...
	 * 	@param trxName trx
	 */
	public HBC_Doc_AllocationHdr (MAcctSchema as, ResultSet rs, String trxName)
	{
		super (as, rs, trxName);
	}   //  Doc_Allocation
		...
		BigDecimal allocationSource)
	{
		...
		HBC_Doc_Invoice docInvoice = (HBC_Doc_Invoice)Doc.get(as,
	...
	private MAccount getPaymentAcct (MAcctSchema as, int C_Payment_ID)
	{
		...
		MPayment payment = new MPayment(getCtx(), C_Payment_ID, getTrxName());
		if(payment.getTenderType().equals(MPayment.TENDERTYPE_Giro)){
			accountType = Doc.ACCTTYPE_UnallocatedGiro;
		}else{
		...
		try
		{
			...
			if (rs.next ())
			{
				...
				m_giroStatus = rs.getString(6);
				setC_BankAccount_ID(rs.getInt(1));
				if (DOCTYPE_APPayment.equals(rs.getString(2)))
					if (!m_tendertype.equals(MPayment.TENDERTYPE_Giro)){ 
					accountType = Doc.ACCTTYPE_PaymentSelect;	
					}else{
					accountType = Doc.ACCTTYPE_PaymentSelectGiro;
