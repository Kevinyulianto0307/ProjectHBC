 ...
 * @author TommyAng 
 */
public class HBC_CalculateAnnual2016 extends SvrProcess{
	...
	/**global variable*/
	protected int HBC_Tugboat_ID = 0;
	protected int HBC_Barge_ID = 0;
	...
	protected void createARCalculationLineLoadingOnly(MARCalculation calc){
		...
		for (int HBC_Trip_ID : getTripIDs()) {
			...
			//for (X_AR_CalculationTemp temp : getTemp(calc.getHBC_Contract_ID())) {
			for (int id : calcTempID) {
				/*
				StringBuilder where = new StringBuilder();
				where.append("HBC_Trip_ID="+temp.getHBC_Trip_ID());
				
				boolean match = new Query(getCtx(), MARCalculationLine.Table_Name, where.toString(), get_TrxName())
				.setClient_ID()
				...
				*/
				...
				calcLine.setAmountOfCargo(hbccontract.getAmountOfCargo());
				...
				//calcLine.setTotalCargoQty((BigDecimal) temp.get_Value("TotalCargoQty"));
				BigDecimal qtyActivity = temp.getAmountOfCargo();
				BigDecimal qtyCharge = qtyActivity;
				
				if(hbccontract.getAmountOfCargo().compareTo(qtyCharge)>0)
					qtyCharge = hbccontract.getAmountOfCargo();
				
				
				/*
				if(hbccontract.getAmountOfCargo().compareTo(qtyCharge)>0)
					calcLine.setQtyCharge(hbccontract.getAmountOfCargo());
				else
				...
				*/
				...
				//MProject project = new MProject(getCtx(), position.getC_Project_ID(), get_TrxName());
				
				//int From_PortPosition_ID = temp.getHBC_PortPosition_ID();
				...
				//calcLine.setFrom_PortPosition_ID(temp.get_ValueAsInt("From_PortPosition_ID"));
				calcLine.setFrom_PortPosition_ID(From_PortPosition_ID);
				calcLine.setTo_PortPosition_ID(To_PortPosition_ID);
				//calcLine.setTo_PortPosition_ID(temp.get_ValueAsInt("To_PortPosition_ID"));
				
				
				BigDecimal price = Env.ZERO;
				//BigDecimal price = getPrice(temp.getHBC_Trip_ID(), temp.get_ValueAsInt("From_PortPosition_ID"), temp.get_ValueAsInt("To_PortPosition_ID"), (Timestamp) temp.get_Value("finishDate"));
				//price = getPrice(temp.getHBC_Trip_ID(), From_PortPosition_ID, temp.get_ValueAsInt("To_PortPosition_ID"), (Timestamp) temp.get_Value("finishDate"));
				price = getPrice(temp.getHBC_Trip_ID(), From_PortPosition_ID, To_PortPosition_ID, (Timestamp) temp.get_Value("finishDate"));
				BigDecimal totalAmt = Env.ZERO;
				//totalAmt = ((BigDecimal) temp.get_Value("TotalCargoQty")).multiply(price);
	...
	protected void createARCalculationLineAnnual(MARCalculation calc){
		...
		for (int HBC_Trip_ID : getTripIDs()) {
			...
			int[] calcTempID = getTempIDs(HBC_Trip_ID);
			boolean minimumCargo = false;
			BigDecimal currentAmt = Env.ZERO;
			int targetID = 0;
			BigDecimal currentLoadingAmt = Env.ZERO;
			int fromPortID = 0;
			...
			//int count = 0;
				...
				for (int id : calcTempID) {
					...
					if(loadTemp.getC_Activity().getValue().equals(LOADING)){
						fromPortID = loadTemp.getHBC_PortPosition_ID();
						currentLoadingAmt = loadTemp.getAmountOfCargo();
						
						for(int id1 : calcTempID){
							X_AR_CalculationTemp dischargingTemp = new X_AR_CalculationTemp(getCtx(), id1, get_TrxName());
							if(dischargingTemp.getC_Activity().getValue().equals(DISCHARGING)){
								BigDecimal price = Env.ZERO;
								//price = getPrice(HBC_Trip_ID, fromPosition, toPosition, (Timestamp) calcTemp.get_Value("finishDate")); @TommyAng
								
								MARCalculationLine calcLine = new MARCalculationLine(getCtx(), 0, get_TrxName());
								calcLine.setHBC_ARCalculation_ID(calc.getHBC_ARCalculation_ID());
								calcLine.setAD_Org_ID(dischargingTemp.getAD_Org_ID());
								//calcLine.setQtyCharge(calcTemp.getAmountOfCargo());
								calcLine.setAmountOfCargo(contract.getAmountOfCargo()); //TODO : change to barge
								calcLine.setLine(line+=10);
							
							
								//calcLine.setFrom_PortPosition_ID(fromPosition);
								calcLine.setFrom_PortPosition_ID(fromPortID);
								calcLine.setTo_PortPosition_ID(dischargingTemp.getHBC_PortPosition_ID());
							
								BigDecimal qtyCharge = dischargingTemp.getAmountOfCargo();
								
								if(qtyCharge.compareTo(currentLoadingAmt)>0)
									qtyCharge = currentLoadingAmt;
								BigDecimal returnCargo = (BigDecimal) dischargingTemp.get_Value("CargoReturn");
								
								if(returnCargo==null)
									returnCargo = Env.ZERO;
								qtyCharge = qtyCharge.add(returnCargo);
								
								//hadi said doesn't care about anything else outside the scenario
								if(contract.getAmountOfCargo().compareTo(qtyCharge)>0)									
									minimumCargo = true;
								
								//if(contract.getAmountOfCargo().compareTo(qtyCharge)>0)
								//	qtyCharge = contract.getAmountOfCargo();
								BigDecimal qtyToInvoice = qtyCharge;	
								BigDecimal qtyActivity = qtyCharge;
								price = getPrice(HBC_Trip_ID, fromPortID, dischargingTemp.getHBC_PortPosition_ID(), (Timestamp) loadTemp.get_Value("FinishDate"));
								...
								HBC_Tugboat_ID = dischargingTemp.get_ValueAsInt("HBC_Tugboat_ID");
								HBC_Barge_ID = dischargingTemp.get_ValueAsInt("HBC_Barge_ID");

								calcLine.setTotalCargoQty(qtyActivity);
								//calcLine.set_ValueOfColumn("DateStart", dischargingTemp.get_Value("StartDate"));
								//calcLine.set_ValueOfColumn("EndDate", dischargingTemp.get_Value("FinishDate"));
								calcLine.set_ValueOfColumn("DateStart", loadTemp.get_Value("StartDate"));
								calcLine.set_ValueOfColumn("EndDate", loadTemp.get_Value("FinishDate"));
								...
								calcLine.setC_Activity_ID(loadTemp.getC_Activity_ID());
								calcLine.setARCalculationType(MARCalculationLine.ARCALCULATIONTYPE_Annual);
								calcLine.setM_Product_ID(M_Product_ID);
								calcLine.saveEx();
								
								if(returnCargo.compareTo(Env.ZERO)>0)
									fromPortID = dischargingTemp.getHBC_PortPosition_ID();
								
								if(qtyCharge.compareTo(currentAmt)>0){
									targetID = calcLine.get_ID();
									currentAmt = qtyCharge;
								}
							}
						}
					}
				...
				//@TommyAng
				//request by hadi lukman
				if(minimumCargo){					
					MARCalculationLine targetedLine = new MARCalculationLine(getCtx(), targetID, get_TrxName());
					if(contract.getAmountOfCargo().compareTo(targetedLine.getQtyCharge())>0){	
						targetedLine.setQtyCharge(contract.getAmountOfCargo());
						targetedLine.setQtyToInvoiced(targetedLine.getQtyCharge());
						BigDecimal totalAmt = targetedLine.getQtyToInvoiced().multiply(targetedLine.getUnitPrice());
						if(contract.getC_Currency_ID()==currencyID)
							targetedLine.setTotalAmt(totalAmt.setScale(0, RoundingMode.HALF_UP));
						else
							targetedLine.setTotalAmt(totalAmt.setScale(2, RoundingMode.HALF_UP));
					}
					targetedLine.saveEx();
				}
		...
		try{
			...
			while(rs.next()){
				...
				if(loading){
					...
					//temp.setTotalCargoQty(rs.getBigDecimal(3));
				}else{
	 ...
	 */
	protected BigDecimal getPrice(int HBC_Trip_ID, int fromPosition, int toPosition, Timestamp finishDate){
		MTrip trip = new MTrip(getCtx(), HBC_Trip_ID, get_TrxName());
		...
		//StringBuilder tempNoPrice = new StringBuilder();
		...
		if(priceStd == null || priceStd.compareTo(Env.ZERO) <= 0){
			...
			throw new AdempiereException("There is no price from "+fromPortPosition.getName()+" to "+toPortPosition.getName()
					...
					+", BargeCategory "+bargeCat.getName());
		}
