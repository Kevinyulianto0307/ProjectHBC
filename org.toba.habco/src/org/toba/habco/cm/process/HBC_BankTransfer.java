 ...
 *	
 **/
public class HBC_BankTransfer extends SvrProcess
{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			*/
			...
			//else if (name.equals("DateAcct"))
				...
				p_NoVoucher = para[i].getParameterAsString();
			else
	...
	private void generateBankTransfer()
	{
		...
		paymentBankFrom.setDateAcct(p_StatementDate);
		...
		paymentBankFrom.set_CustomColumn("NoVoucher", p_NoVoucher);
		...
		paymentBankTo.setDateAcct(p_StatementDate);
		...
		paymentBankTo.set_CustomColumn("NoVoucher", p_NoVoucher);
		...
		if(mBankFrom.getC_Currency().getISO_Code().equalsIgnoreCase("IDR")){
			
			StringBuilder sb = new StringBuilder();
			sb.append("UPDATE C_Payment SET PayAmt=ROUND(PayAmt,0) WHERE C_Payment_ID=?");
			int no = DB.executeUpdate(sb.toString(), paymentBankFrom.get_ID(), get_TrxName());
		...
		if(mBankTo.getC_Currency().getISO_Code().equalsIgnoreCase("IDR")){
			
			StringBuilder sb = new StringBuilder();
			sb.append("UPDATE C_Payment SET PayAmt=ROUND(PayAmt,0) WHERE C_Payment_ID=?");
			int no = DB.executeUpdate(sb.toString(), paymentBankTo.get_ID(), get_TrxName());
			log.info("UPDATED PayAmt C_Payment#"+no);
