...
public class HBC_ProjectIssue extends SvrProcess{
	...
	private boolean	p_IsService = false;
	
	private BigDecimal p_PriceEntered = Env.ZERO;
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("IsService"))
				p_IsService = para[i].getParameterAsBoolean();
			else if (name.equals("PriceEntered"))
				p_PriceEntered = para[i].getParameterAsBigDecimal();
			else
	...
	private void calculateProject (){
		if(m_C_ProjectLine_ID > 0){
			...
			ProductCost pc = new ProductCost(getCtx(), projectLine.getM_Product_ID(), 0, get_TrxName());
			pc.setQty(Env.ONE);
			BigDecimal currentCost = (p_IsService && p_PriceEntered.compareTo(Env.ZERO) > 0)
					? p_PriceEntered
					: pc.getProductCosts(as, 0, as.getCostingMethod(), 0, true);
			projectLine.set_ValueOfColumn("CommittedPrice", currentCost);
			
			BigDecimal commitQty = projectLine.getCommittedQty();
			BigDecimal commitAmt = currentCost.multiply(commitQty).setScale(2, RoundingMode.HALF_UP);
			projectLine.setCommittedAmt(commitAmt);
			
			projectLine.saveEx();
			
			BigDecimal accumProjectBalance = m_project.getProjectBalanceAmt();
			accumProjectBalance = accumProjectBalance.add(commitAmt);
			m_project.setProjectBalanceAmt(accumProjectBalance);
			m_project.saveEx();
		}
		else{
			for (MProjectLine projectLine : m_project.getLines()) {
				MClientInfo m_clientInfo = MClientInfo.get(getCtx(), getAD_Client_ID(), get_TrxName());
				MAcctSchema as = new MAcctSchema(getCtx(), m_clientInfo.getC_AcctSchema1_ID(), get_TrxName());
