 ...
 *****************************************************************************/
package org.toba.habco.webui.apps.form;
...
//	private int			i_multiplier = 10;
	...
	{		
		...
		StringBuilder sql = new StringBuilder("SELECT p.DateTrx,p.DocumentNo,p.C_Payment_ID,p.DocStatus,  "  //  1..4
			...
			+ "p.MultiplierAP, cd.name, p.NoVoucher , tb.name, br.name "
			+ "FROM C_Payment_v p"		//	Corrected for AP/AR
			+ " INNER JOIN C_Currency c ON (p.C_Currency_ID=c.C_Currency_ID) "
			//@tegar
			+ " INNER JOIN C_DOctype cd ON (cd.C_Doctype_ID = p.C_Doctype_ID) "
			//tegar
			//@phie
			+ " LEFT JOIN HBC_Tugboat tb ON (tb.hbc_tugboat_id = p.hbc_tugboat_id) "
			+ " LEFT JOIN HBC_Barge br ON (br.hbc_barge_id = p.hbc_barge_id) "
			//end phie
		...
		try
		{
			...
			while (rs.next())
			{
				...
				BigDecimal available = rs.getBigDecimal(8);
				...
				//@phie
				line.add(rs.getString(12));							// Tugboat
				line.add(rs.getString(13));							// Barge
				//end phie
	...
	public void setPaymentColumnClass(IMiniTable paymentTable, boolean isMultiCurrency)
	{
		...
		paymentTable.setColumnClass(i++, String.class, true);			// 11 - Tugboat
		paymentTable.setColumnClass(i++, String.class, true);			// 12 - Barge
	...
	public Vector<Vector<Object>> getInvoiceData(boolean isMultiCurrency, Object date, IMiniTable invoiceTable)
	{
		...
		StringBuilder sql = new StringBuilder("SELECT i.DateInvoiced,i.DocumentNo,i.C_Invoice_ID,i.DocStatus,  " //  1..4
			...
			+ " i.MultiplierAP,cd.name, i.OrderReference , tb.name , br.name, i.dateacct"
			...
			//end
			// @phie
			+ " LEFT JOIN HBC_Tugboat tb ON (tb.hbc_tugboat_id = i.hbc_tugboat_id) "
			+ " LEFT JOIN HBC_Barge br ON (br.hbc_barge_id = i.hbc_barge_id) "
			//end phie
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//@phie
				line.add(rs.getString(13));						// Tugboat
				line.add(rs.getString(14));						// Barge
				line.add(rs.getTimestamp(15));					// DateAcct
				//end phie
	...
	public void setInvoiceColumnClass(IMiniTable invoiceTable, boolean isMultiCurrency)
	{
		...
		invoiceTable.setColumnClass(i++, String.class, true);			// Tugboat
		invoiceTable.setColumnClass(i++, String.class, true);			// Barge
